...
their *rendering* is not a promise. Trailing zeros in the fraction are trimmed, so
the number of decimals varies with the value — the same code rendered `.6994Z` in CI
and `.532982Z` on a workstation — and the representation is not guaranteed to be a
string at all.
